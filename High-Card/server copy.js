...
app.get('/api/draw', async (req, res) => {
  try {
    ...
    const [playerCard, computerCard,playerCard2,computerCard2] = drawData.cards; // プレイヤーとコンピューターのカードを取り出す


    // 絵札（J・Q・K・A）を数字に変えるための関数
    const getCardValue = (value) => {
      if (['KING', 'QUEEN', 'JACK'].includes(value)) return 0;
    ...
    let plscore = (parseInt(playerValue) + parseInt(playerValue2))%10;
    let coscore = (parseInt(computerValue) + parseInt(computerValue2))%10;
    ...
    plscore = 'score' + plscore;
    coscore = 'score' + coscore;
